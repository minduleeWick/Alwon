...
const Inventory: React.FC = () => {
  ...
  const handleDelete = async () => {
    try {
      ...
      await axios.delete(`http://localhost:5001/api/inventory/${currentItem._id}`);
  ...
  const handleSubmit = async (items: InventoryItem[]) => {
    try {
      ...
      if (_id && _id !== '') {
        ...
        await axios.put(`http://localhost:5001/api/inventory/${_id}`, payload);
        setSnackbar({ open: true, message: 'Inventory updated!', severity: 'success' });
        setEditOpen(false);
      } else {
        // Add mode: Call POST endpoint
        await axios.post('http://localhost:5001/api/inventory/add', payload);
